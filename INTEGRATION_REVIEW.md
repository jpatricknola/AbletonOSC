...
### Echo-discarding reads (Seshat side) — *corrected 2026-08-03*

An earlier revision of this section called the regular-track device reads "the
only multi-index reads in the file that skip the echo check." **That was
wrong** — subsequent review (PR #62) found five site families that discard
correlation data on a transport that correlates by address alone:

1. `get_track_devices` / `get_device_parameters` (regular tracks) — 3–5
   parallel list replies, echoed track index discarded; the replies can
   describe two different chains. The hazard the vendored combined
   return/master endpoints exist to close.
2. `get_clip_notes` — three separate replies with echoed track/slot discarded;
   a straggler can pair one clip's name with another clip's length or notes.
3. `set_device_parameter` (regular track) — the *confirming read* discards all
   three echoed indices, so it can present another parameter's display value
   as verification.
4. `query_scene_names/1` — discards the echoed scene index.
5. `list_browser_items` — discards the echoed category/filter, on the wide
   15s browse timeout window.

All five are now marked `TODO!` in Seshat's `handlers.ex`. The durable fix is
echo-aware reply decoding shared by every raw `Transport.query` call site —
or, for (1), the combined regular-track endpoints proposed in §3.
...
5. **Add echo checks (or adopt the future combined endpoints) at all five
   echo-discarding site families** listed in §2 — regular-track device reads,
   `get_clip_notes`, `set_device_parameter`'s confirming read,
   `query_scene_names/1`, and `list_browser_items`. All five carry `TODO!`
   markers as of the PR #62 review.
...
7. **One stale rationale, three places** *(all fixed 2026-08-03)*: the
   browser-export age gate was justified with "the transport does not
   serialize queries" in Seshat's `vendored_addresses_test.exs`, this fork's
   `browser.py:138`, and SESHAT.md's browser section. Transport *does*
   serialize queries; the gate is still load-bearing because the caller reads
   the exported file only after its reply arrives, outside any transport
   ordering, so a later export's pre-sweep can race that read. All three
   comments now state that rationale.

### 4a. Full doc-coverage diff (`docs/abletonosc-api-docs.md` vs. registered addresses)

> **Status:** this diff is a snapshot taken 2026-08-03 against the doc as it
> stood *before* the Seshat-side documentation PR. That PR has since begun
> absorbing these gaps (`scenes/name`, the Link property, the device listener
> section, and others are now documented). Re-run the extraction before
> treating any row below as still open; the durable fix — extending the
> doc-coverage test to upstream registrations — is what to verify landed.
...
- **device** — the `class_name`/`name`/`type` listener pairs and
  ...
  ⚠️ *Follow-up (PR #62 review):* the three property-listener pairs turned out
  to be **defective as registered**, not merely undocumented — see §5 below.
  Seshat's doc now documents the actual hobbled behavior; the fork-side fix is
  issue #23 in [issues.md](issues.md).
...
## 5. Fork defect found in review follow-up: device property listeners lose identity

*(Added 2026-08-03 after the PR #62 review; verified against
[device.py](abletonosc/device.py) and [handler.py](abletonosc/handler.py).)*
...
`include_ids=True` ([device.py:38-44](abletonosc/device.py#L38-L44)). The
wrapper strips the two indices before the base class sees them
([device.py:10-17](abletonosc/device.py#L10-L17)), so `_start_listen`
receives `params = ()`, which has two consequences
([handler.py:73-97](abletonosc/handler.py#L73-L97)):

1. **The push carries the bare value only** — `(*params, *value)` with empty
   `params` means no track or device echo on the `get/` address, so a client
   cannot tell which device changed.
2. **The listener key is `(prop, ())` for every device**, so subscribing a
   second device to the same property silently stops and replaces the first.
   One subscription per property, process-wide.

`start_listen/parameter/value` ([device.py:158-159](abletonosc/device.py#L158-L159))
already does this correctly with `include_ids=True`. The fix is the same flag
on the three property-listener registrations — note this changes both the push
shape (ids prepended) and the subscription identity, so it is a contract
change for any existing subscriber, and it needs the listener-lifecycle
regression coverage that issue #9 also calls for. Tracked as **issue #23**.
Until it lands, Seshat's API doc deliberately documents the hobbled behavior
and warns against building on these three pairs.

## 6. What was explicitly considered and rejected
