...
| `/live/view/focus_view` | `view_name` | | ⚠️ **Seshat extension** — give a pane keyboard focus. Distinct from `show_view`, which only makes it visible. Live's menu-command validation reads focus: measured 2026-08-30, Create → Convert Melody to New MIDI Track stayed disabled after `show_view("Session")` with a verified audio clip selected over OSC, and enabled only after the Session grid was clicked. No reply. ⚠️ **The accepted name set is assumed, not measured.** Live's signature is `focus_view((View)arg1, (object)arg2)`, identical to `show_view`/`hide_view`, so this fork passes the same names through and rejects none itself — but only `Browser` has been seen accepted (2026-08-30, via its effect on the Convert commands). What *focusing* each name does is unmeasured too, and `hide_view` is the warning: it accepts all six names while only `Browser` and `Detail` truly hide. An unrecognised name is logged and silent, like the other two steers |
