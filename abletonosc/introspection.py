...
nested classes such as Live.Song.Song.View) and records each member's kind:
...
def walk_live():
    """
    Return {qualified_class_name: {member_name: info}} for every class
    reachable from the Live module.
    """
    import Live
    classes = {}
    seen = set()

    def visit_class(cls, qualname):
        if id(cls) in seen:
            return
        seen.add(id(cls))
        members = {}
        for name in dir(cls):
            ...
            if isinstance(attr, type):
                visit_class(attr, qualname + "." + name)
        classes[qualname] = {"doc": _doc(cls), "members": members}

    def visit_module(mod, qualname):
        if id(mod) in seen:
            return
        seen.add(id(mod))
        for name in dir(mod):
            ...
            if inspect.ismodule(attr):
                visit_module(attr, qualname + "." + name)
            elif isinstance(attr, type):
                visit_class(attr, qualname + "." + name)

    visit_module(Live, "Live")
    ...
    try:
        ...
        for cls in AVAILABLE_TYPE_PROPERTIES:
            if isinstance(cls, type):
                visit_class(cls, cls.__module__ + "." + cls.__name__)
    except Exception as e:
