...
def create_track_callback(get_tracks: Callable[[], Any],
    ...
    Args:
        ...
        include_track_id: Hand `func` the subscription's identity — the
                          one-int tuple `(track_index,)` — in place of the
                          params tail, as the listener registrations need.
                          The tail is deliberately excluded: it is not part
                          of the identity, so it must reach neither the
                          bookkeeping key nor the push.
    ...
    def invoke(track_index: int, params) -> Optional[Tuple[Any, ...]]:
        ...
        if include_track_id:
            ...
            # OSC args. Every include_track_id callee is a listener registration,
            # and a listener's identity has to be canonical: it is the bookkeeping
            # key, the LOM subscript and the echo in every push, and those three
            # must agree across a start/stop pair sent by different clients.
            # `track_index` is already the int this wrapper computed — cast from
            # params[0] on the concrete path, generated by range() on the wildcard
            # path. A track subscription's identity is exactly that one int:
            # anything past it is dropped here, so a stray trailing argument
            # cannot key a second subscription that a well-formed stop can never
            # reach, nor ride along as a bogus field on every push.
            #--------------------------------------------------------------------------------
