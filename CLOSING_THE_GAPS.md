...
| ~~**D-2** Groove~~ | ~~`/live/song/get/groove_pool` → indexed names and amounts; `Groove.*` amounts get/set; `/live/clip/get|set/groove` by pool index or `-1`. Measure whether `browser.load_item` can load an `.agr` into the pool.~~ Landed 2026-08-29, branch `feat/groove`. Closed [Groove Pool](FORK_GAPS.md#groove-pool--closed-2026-08-29) (now `FORK_GAPS.md` § Closed), including the curated `Clip.groove` entry it named. `.agr`-via-browser measurement carried forward as still open — see that section. |
