...
|---|---|---|
| **Object view classes** | The per-object view resolver is the substance; the members are cheap once it exists. `Song.View`: `draw_mode`, `follow_song`, `select_device`. `Application.View`: `available_main_views`, `browse_mode`, `scroll_view`, `zoom_view`, `toggle_browse` — per the cautions, document `toggle_browse` as overlapping the absolute `show_view`/`hide_view` pair, and measure the two `*_view` argument forms before writing them. `Track.View`: `is_collapsed`, `device_insert_mode`, `select_instrument`. `Clip.View`: `grid_quantization`, `grid_is_triplet`, the envelope show/hide four; plus the `Clip.view` member itself. (Three members have **shipped** ahead of the resolver, each because a focus measurement made it urgent: `Application.View.focus_view` as `/live/view/focus_view`, and `focused_document_view` and `Song.View.highlighted_clip_slot` as the reads that check it — `/live/view/get/focused_document_view` plus a listen pair, and `/live/view/get|set/highlighted_clip_slot`. `focus_view` was previously written off here as overlapping `show_view`; it does not — show makes a pane visible, focus gives it keyboard focus, and Live's menu-command validation reads focus. The `focused_document_view` **High** disposition is closed.) Closes the [View addressing gap](FORK_GAPS.md#songview--applicationview--liveview-is-a-fixed-set). This is the bucket the dispositions table names. | 18 |
